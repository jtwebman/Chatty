...
function joinRoom(socket, room) {
	...
	socket.emit('joinResult', { room: room, nickname: nickNames[socket.id] });
...
function handleRoomJoining(socket) {
	socket.on('join', function(room) {
		oldroom = currentRoom[socket.id];
		socket.to(oldroom).emit('message', { text: nickNames[socket.id] + ' has left ' + oldroom + '.' });
